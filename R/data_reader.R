...
#' @description read_methylome is a function to import
#' methylation data into a GRanges
#' object. Bed file should be in EPP,ALLC or BisSNP format.
#' @param filename - filename which contains methylation data
#' @param type Type of file format. Currently supported
#' epp, bissnp, bismarkCytosine,
...
#' @return a \code{GenomicRange} object with methylation,
...
read_methylome <- function(filename,
  ...
  if (!file.exists(filename)) {
    stop(paste(filename, " doesn't exist or
        path is incorrect!"))
  }
  if (!type %in% c(
    "bissnp", "epp", "allc",
    "bismarkcytosine", "bismarkcov", "encode"
  )) {
    stop(paste(type, " is not a valid file type!"))
  }
  if (!is.numeric(cov_threshold) || cov_threshold < 0) {
    stop(paste(cov_threshold, " is not a
         valid coverage threshold!"))
  }
  if (type == "epp") {
    # Parse EPP file format
    msites <- data.table::fread(filename,
  ...
  if (type == "allc") {
    ...
    allc <- data.table::fread(filename,
      header = FALSE, showProgress = FALSE
    )
    if (ncol(allc) < 6) {
      logger::log_warn(paste0(
        filename,
        " is an invalid allc file!"
      ))
      stop("allc file must contain at
             least 6 columns!")
    }
    cov <- allc$V6
  ...
  if (type == "bismarkcytosine") {
    ...
    if (ncol(msites) < 5) {
      logger::log_warn(paste0(filename, "
            is an invalid bismarkCytosine file!"))
      stop("bismarkCytosine file must
            contain at least 5 columns!")
    }
  ...
  if (type == "bismarkcov") {
    ...
    if (ncol(msites) < 6) {
      logger::log_warn(paste0(filename, "
            is an invalid bismarkCov file!"))
      stop("bismarkCov file must contain
            at least 6 columns!")
    }
    mscore <- round(msites$V4 / 100, 6)
...
#' @description  helper function to convert
#' msites in EPP GRanges format
...
#' @keywords internal
granges_helper <- function(grobj, chr, mscore, cov, # meth,
                           startP, endP) {
