...
#' @param bootstrap_error logical, if TRUE bootstrap confidence bounds for the
...
#' \code{bootstrap_upper_bound} when \code{bootstrap_error} is TRUE. Rows are
...
computeZScoreVariability <- function(object,
    ...
    if (!is.logical(bootstrap_error) || length(bootstrap_error) != 1) {
        stop("bootstrap_error must be a single logical value")
    }
