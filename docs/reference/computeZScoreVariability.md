...
[`chromVAR::computeVariability`](https://rdrr.io/pkg/chromVAR/man/computeVariability.html),
with one difference: chromVAR calibrates each motif against a set of GC-
and width-matched background peak sets, whereas here the null is
estimated across motifs within each sample. The latter needs no
additional deviation computations and can be applied to existing
results, at the cost of assuming that most motifs are inactive in any
given sample.
